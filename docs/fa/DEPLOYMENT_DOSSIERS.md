...
پیش از نصب برنامه توقف کنید. اکنون API در سطح مبدأ، مهاجرت و نقش‌های گروهی PostgreSQL، رفتار ماندگار worker با دادهٔ آزمون، اسکریپت‌های محافظت‌شده برای لایهٔ آفلاین و معتبر بسته‌ها و پروفایل مبدأ systemd برای هوش مصنوعی وجود دارد. بااین‌حال بستهٔ تأییدشده، نصب‌کننده یا انتشار کامل تولید، رابط مرورگر، تحویل نقش ورود تولید، reverse proxy، Compose، واحدهای مستقر برنامه و رویهٔ آزمودهٔ پشتیبان‌گیری و بازیابی هنوز آماده نیستند. وجود واحدهای هوش مصنوعی شاهد نصب نیست. موفقیت نصب بسته را استقرار محصول ندانید، فرمان CI را به دستورالعمل تولید تبدیل نکنید و برای جبران، متغیر محیطی ساختگی یا PostgreSQL شبکه‌ای نسازید.
