...
**وضعیت: قرارداد تحویل همراه installer محافظت‌شدهٔ OS-package؛ نه installer کامل محصول و نه مجوز استقرار.** چهار پروندهٔ YAML اندازه، مرز، وابستگی، مسیر، الگوی فرمان و مرجع ورودی خصوصی را حفظ می‌کنند و scriptها فقط لایهٔ package آفلاین را پوشش می‌دهند. یک استقرار کنترل‌شدهٔ جداگانه اکنون دروازه‌های ثبت‌شده در [وضعیت انتشار](../status/current-release.yaml) را گذرانده است؛ این شواهد installer بسته‌ها را به installer کامل محصول تبدیل نمی‌کند.
...
پیش از استفاده از این پرونده‌ها به‌عنوان installer برنامه توقف کنید. مخزن API/UI، نقش‌های PostgreSQL، گردش ماندگار، لایهٔ محافظت‌شدهٔ بسته و پروفایل systemd هوش مصنوعی را دارد و یک استقرار کنترل‌شدهٔ جدا نیز صلاحیت‌سنجی شده است. بااین‌حال بستهٔ کامل تولید، تحویل اعتبارنامهٔ خصوصی، محیط reverse proxy، روش مستقل پشتیبان/WAL/PITR و مجوز تولید در این پرونده‌ها نیست. موفقیت نصب package را استقرار محصول ندانید، فرمان CI را runbook تولید نکنید و PostgreSQL را برای جبران روی شبکه باز نکنید.
