...
NextOps has a controlled four-guest deployment with immutable releases, local identity/session handling, PostgreSQL state, durable runs/audit, a bilingual panel, CPU-only model execution and an isolated read-only Zabbix connector. Phase 2A adds a deployed, fixed-target incident-context route with bounded history/events and an exact five-method reader allowlist; its durable model/browser use remains unaccepted. Production acceptance is still blocked by independent backup, WAL/PITR and disaster-recovery evidence. The highest-risk boundaries remain release/model integrity, isolation and scope of connector credentials, target substitution and prompt injection, deployer secret handling, and host-loss recovery. The deployment remains one organization on an internal LAN/VPN with no intended public ingress; future multi-organization mode requires a separate isolation design and test gate.
...
- Current implementation reality: the locked Python project and controlled deployment include typed
  contracts, deterministic policy, FastAPI services, local Argon2id identity and opaque sessions,
  PostgreSQL/Alembic state, durable idempotent runs and leases, append-restricted audit, a bilingual
  browser panel, immutable systemd releases, a pinned CPU model/runtime and a credential-isolated
  Zabbix connector. This is controlled user testing, not production acceptance.
...
- Production bootstrap/recovery secret delivery, optional LAN identity-provider relationship, and certificate/time source.
...
- `nextops-app`: source-level local API, identity, deterministic policy calls, durable fixture-run orchestration, and audit initiation; reverse proxy, browser UI and deployment remain proposed.
- PostgreSQL in `nextops-app`: implemented initial schema, constraints, group roles, durable jobs, and append-restricted audit; production login roles, patch selection, backup and deployment remain open.
...
- Authorized browser/CLI -> `nextops-app`: implemented bounded session/run request schemas and revocation; certificate-verified HTTPS, reverse proxy, rate limits and browser CSRF/origin controls remain deployment/UI work.
- `nextops-app` -> PostgreSQL: implemented identities, scopes, runs, leases, fixture evidence metadata and audit with restricted group roles, migrations, scoped constraints and transaction boundaries; production login/secret delivery remains open.
...
|---|---|---|---|---|
| Web/API question submission | Internal browser or CLI | User -> app | Minimal authenticated, versioned, bounded fixture-run API implemented; browser UI not implemented | `packages/nextops/api/app.py`; `packages/nextops/contracts/durable.py` |
| Session and recovery endpoints | Internal browser/admin path | User/admin -> identity | One-time bootstrap, login and recovery/revocation implemented; production secret delivery remains open | `packages/nextops/application/service.py`; `tests/integration/test_durable_app_postgresql.py` |
...
| PostgreSQL roles and migrations | Internal services and release process | Service/release -> database | Baseline, constraints, restricted group roles and append-only trigger implemented and CI-tested; deployment login roles/serialization remain | `migrations/versions/0001_durable_app.py`; `tests/integration/test_durable_app_postgresql.py` |
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| TM-001 | Authenticated limited user | Valid session and an object ID outside the user's scope | Manipulates target/scope fields or replays another run | Unauthorized evidence disclosure or work initiation | Identity, scopes, evidence, audit | Server-derived actor scope, composite scope keys/FKs, policy checks and denied cross-environment test are implemented | Real connector/evidence retrieval and future multi-org paths are not implemented | Preserve organization/environment/target scope in every permission-sensitive key; re-authorize before collection and evidence retrieval; extend adversarial tests with each domain | Audit denied target IDs and repeated scope mismatches without logging sensitive payloads | Medium | High | High |
...
| TM-005 | Service failure or privileged operator | Audit/database unavailable or bypass route exists | Executes or serves evidence without durable policy/audit record | Loss of accountability and hidden abuse | Audit, workflow state, evidence access | State/audit share transactions; role grants and a trigger reject mutation; rollback failure is tested on PostgreSQL | External checkpoints, partition/retention, browse/export and privileged-host resistance are not implemented | Keep required paths fail-closed; add protected external checkpoints, retention monitoring and recovery tests before production | Alert on write failures, sequence gaps, clock anomalies, and unlogged result paths | Medium | High | High |
...
| TM-007 | Compromised contributor, registry, or provisioning workstation | Ability to alter a release/model artifact before import | Supplies tampered code, image, model, template, or dependency | Code execution, evidence exfiltration, or answer corruption across all services | Release bundle, runtime, model, credentials | Python lock, pinned CI actions/service image, dependency audit and full-history secret scan are implemented | No release signing, SBOM, model manifest, offline bundle or verifier exists | Generate SBOM/model manifests; sign release metadata; verify checksums/licenses offline before promotion; keep credentials out of bundles | Record bundle identity and verification result; alert on mismatch or unexpected outbound request | Medium | High | High |
...
| TM-009 | Connector failure, stale item, permission gap, or model error | Partial or old evidence is returned without complete metadata | Presents missing or stale results as current healthy status | Incorrect operational decision | Evidence provenance and answer integrity | Live bounded Zabbix summary and incident-context contracts preserve source/time/partial/stale metadata; prompt-injection text and dependency failure paths are tested | The new incident context is not yet part of the durable model/browser path and broader answer regression evaluation remains | Preserve metadata through collection and display; compute counts in code; require explicit unknown/stale states; add durable incident-context evaluation | Compare answer facts to captured evidence; monitor stale/partial response rates | High | Medium | High |
...
- Critical: a reachable unauthenticated path to connector credentials or remote code execution; a policy bypass that enables infrastructure mutation; or systematic cross-organization disclosure after expansion. The deployed connector owns its credential behind authenticated named reads; no infrastructure mutation path is enabled.
...
| `.github/workflows/ci.yml`, `migrations`, `packages/nextops`, `tests` | Pinned isolated CI; PostgreSQL migration/roles; local identity, API, run/lease/audit implementation; unit/API/real-PostgreSQL tests; no target connector | TM-001, TM-002, TM-005, TM-007, TM-009 |

## Notes on use

- The Stage 1A identity/database/API controls described above are implemented and tested;
  target, model, deployment, offline-runtime, backup/restore, and later mutation controls
  remain design requirements until their acceptance evidence exists.
- All discovered planned entry points and trust boundaries are represented above; current executable scope includes the local fixture-backed API and isolated PostgreSQL tests, but no deployed or live-target path.
