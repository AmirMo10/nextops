...
NextOps now has a runnable source-level Stage 1A API with immutable contracts, deterministic denial, local identity/session handling, PostgreSQL state, durable runs/leases, append-restricted audit, and a bilingual fixture result. These controls are tested in isolated CI but are not deployed or accepted on a server. Browser UI, connector isolation, operational credentials, model execution, offline release, backup/restore, and production network controls remain proposed. The highest-risk future boundaries are model artifact/runtime integrity, isolation of the Zabbix credential inside the connector runner, prevention of target substitution and prompt injection, deployer secret handling, and host-loss recovery. The initial deployment is one organization on an internal LAN/VPN with no intended public ingress; future multi-organization mode requires a separate isolation design and test gate.
...
- Current implementation reality: the locked Python project now includes typed contracts,
  deterministic policy, a FastAPI entrypoint, local Argon2id identity and opaque sessions,
  PostgreSQL/Alembic state, durable idempotent runs and leases, append-restricted audit,
  a bilingual fixture result, and isolated CI. There is still no browser UI, deployable
  offline release/service definition, operational credential, target connector, model
  runtime, or production infrastructure access.
...
| TM-009 | Connector failure, stale item, permission gap, or model error | Partial or old evidence is returned without complete metadata | Presents missing or stale results as current healthy status | Incorrect operational decision | Evidence provenance and answer integrity | Fixture result schema carries source/time/scope/partial/stale/errors/audit and explicitly labels non-live data | No real connector, deterministic Zabbix aggregator or model answer validator exists | Preserve metadata through collection and display; compute counts in code; require explicit unknown/stale states | Compare answer facts to captured API fixtures; monitor stale/partial response rates | High | Medium | High |
...
- Critical: a reachable unauthenticated path to connector credentials or remote code execution; a policy bypass that enables infrastructure mutation; or systematic cross-organization disclosure after expansion. The current source runtime has no connector credential or mutation path and is not deployed.
