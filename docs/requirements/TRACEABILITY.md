...
| 25 | LLM abstraction / رابط مدل | `packages/nextops/inference`, `packages/nextops/api/answer_integrity.py` | 1–2 | Pinned local CPU model and bounded scheduler are deployed for controlled testing; an unpromoted source guard rejects truncated replies, while exact-prompt bilingual relevance review remains / مدل محلی و صف محدود در ارزیابی کنترل‌شده مستقرند؛ کنترل کد منبع برای پاسخ ناتمام هنوز مستقر نشده و بازبینی ارتباط پاسخ با پرسش باقی است | I |
...
| 30 | UI / رابط کاربری | `packages/nextops/api/static`; future operations console | 2–8 | Bilingual three-mode panel exists; unpromoted source candidate simplifies it to a question/answer flow with focused filesystem display and collapsible complete evidence; wider operations views remain / پنل سه‌حالتهٔ دوزبانه وجود دارد؛ تغییر مستقرنشده، پرسش‌وپاسخ ساده، نمایش متمرکز فایل‌سیستم و شواهد کاملِ بازشونده را می‌افزاید؛ نماهای گستردهٔ عملیات باقی‌اند | I |
