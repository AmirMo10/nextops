...
Updated: 2026-09-21 — Stage 1B source boundary is implemented and locally tested; the observed Ubuntu 22.04 guests are planned for clean 24.04 replacement before authorized server qualification continues.
...
A sanitized read-only preflight reached the four existing role guests and confirmed their public CPU, memory and disk budgets, but all four ran Ubuntu 22.04 rather than the enforced 24.04 baseline. The NextOps role services were absent. A legacy Zabbix stack was active but did not match the fresh-server profile, and no authenticated API or monitoring-data acceptance call was made. The owner plans to remove these guests and replace them with clean Ubuntu 24.04 installations. Deletion and replacement are operator-planned, not completed repository evidence.

The next bounded change starts only after the owner confirms that the replacement guests are available. Independently verify every new SSH host fingerprint before replacing the old local pins, repeat private read-only preflight, and stop on any sizing, mount, listener, time, virtualization or recovery-path mismatch. Then, under the separate applicable authorizations, build and hash llama.cpp from the pinned source, import and locally hash the pinned Qwen file through controlled staging, and measure fresh Persian and English generation from cold local artifacts. Preserve the repository profile initially—loopback provider, CPU-only/zero offload, one active request, queue depth two, 8K configured context and 1024-token output ceiling. No cloud fallback, target credential, management route, tool/agent/MCP mode, or Zabbix completion claim is allowed.
...
پیش‌بررسی پاک‌سازی‌شده و فقط‌خواندنی به چهار مهمان نقش موجود رسید و بودجهٔ عمومی CPU، حافظه و دیسک را تأیید کرد، اما هر چهار مهمان Ubuntu 22.04 داشتند، نه خط مبنای اجباری 24.04. سرویس‌های نقش NextOps وجود نداشتند. یک stack قدیمی Zabbix فعال بود، اما با profile سرور تازه برابر نبود و هیچ API احرازهویت‌شده یا پذیرش دادهٔ پایش اجرا نشد. مالک برنامه دارد این مهمان‌ها را حذف و با نصب تمیز Ubuntu 24.04 جایگزین کند. حذف و جایگزینی، برنامهٔ اپراتور است و هنوز شاهد تکمیل‌شدهٔ مخزن نیست.

تغییر محدود بعدی فقط پس از تأیید آماده بودن مهمان‌های جایگزین توسط مالک آغاز می‌شود. پیش از جایگزینی pinهای قدیمی، اثرانگشت تازهٔ همهٔ میزبان‌های SSH از مسیر مستقل بررسی، preflight خصوصی فقط‌خواندنی تکرار و در صورت ناسازگاری اندازه، mount، listener، زمان، مجازی‌سازی یا مسیر بازیابی توقف شود. سپس با مجوزهای جداگانهٔ لازم، llama.cpp از منبع ثابت build و hash، فایل ثابت Qwen از staging کنترل‌شده وارد و محلی hash و تولید تازهٔ فارسی و انگلیسی از artifact سرد اندازه‌گیری شود. profile مخزن در شروع حفظ شود: provider روی loopback، اجرای CPU-only و zero-offload، یک درخواست فعال، صف دو موردی، context برابر 8K و سقف خروجی 1024 token. fallback ابری، credential مقصد، مسیر مدیریت، tools/agent/MCP و ادعای تکمیل Zabbix ممنوع است.
