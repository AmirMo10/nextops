...
در ۲۱ سپتامبر ۲۰۲۶، پس از آنکه مالک اثرانگشت‌های تازهٔ Ed25519 را از مسیر مستقل فرستاد و آن‌ها با handshake زنده برابر شدند، پیش‌بررسی پاک‌سازی‌شده و فقط‌خواندنی به هر چهار مهمان جایگزین تمیز رسید. هر چهار مهمان Ubuntu 24.04.5 LTS را زیر VMware با بودجهٔ موردنظر vCPU، حافظه، دیسک مجازی و mountهای جدا اجرا می‌کنند. زمان همگام، VMware Tools و SSH فعال، وضعیت systemd برابر `running` و بدون unit خراب، بستهٔ قابل‌ارتقا یا reboot معلق است؛ UFW فعال است و تنها listener عمومی TCP، SSH است. مهمان AI نیز flagهای لازم SSE4.2، AVX، AVX2، AVX-512F، FMA و BMI2 را دارد. pinهای قدیمی کلید میزبان به‌صورت محلی جایگزین شدند؛ نشانی، اثرانگشت و خروجی خام وارد Git نشده‌اند.

مالک آماده‌سازی متصل از طریق زنجیرهٔ proxy سخت‌گیرانهٔ ازپیش‌تنظیم‌شده را صریحاً مجاز کرد. packageهای مشاهده‌شدهٔ هر نقش بدون ارتقای کلی سیستم‌عامل یا شروع خودکار سرویس نصب شدند: میزبان app دارای PostgreSQL 16.15 و Nginx 1.24 است؛ AI دارای GCC 13.3، CMake 3.28، Ninja 1.11 و OpenBLAS 0.3.26؛ connectors دارای پشتیبانی venv در Python 3.12؛ و Zabbix دارای Zabbix 7.0.30، PostgreSQL 16.15، Nginx 1.24 و PHP 8.3.6 است. هویت‌های سرویس و مسیرهای محافظت‌شده ساخته شدند، اما همهٔ سرویس‌های محصول، پایگاه و وب غیرفعال و disabled هستند؛ هیچ cluster از PostgreSQL وجود ندارد، update یا reboot معلق نیست و SSH تنها listener عمومی است. Docker عمداً نصب نشد، چون طراحی منتخب native package و systemd به آن نیاز ندارد و هیچ سرویس نباید socket کانتینر بگیرد.

commit ثابت llama.cpp به‌صورت Release و CPU-native با OpenMP و OpenBLAS و بدون پیوند GPU ساخته شد و SHA-256 فایل `llama-server` ارتقایافته در manifest ثبت است. فایل Qwen با اندازهٔ ۵٬۰۲۷٬۷۸۳٬۴۸۸ بایت از proxy وارد شد، با SHA-256 ثابت برابر بود و به حجم محافظت‌شدهٔ مدل ارتقا یافت. smoke test موقت با احراز هویت، یک slot و فقط loopback در ۷ ثانیه cold start شد و `READY` را در ۶۳۰ میلی‌ثانیه و `آماده` را در ۱۴۳۱ میلی‌ثانیه برگرداند؛ context برابر 8K، GPU layer برابر صفر و Web UI/slot غیرفعال بود و هیچ process یا listener باقی نماند. این شاهد smoke است، نه benchmark کامل یا پذیرش با اینترنت قطع. تغییر محدود بعدی، unit و auth و configuration بازبینی‌شده به‌همراه ارزیابی کامل دوزبانهٔ load، failure و offline است. fallback ابری، credential مقصد، مسیر مدیریت، tools/agent/MCP و ادعای تکمیل Zabbix ممنوع است.
