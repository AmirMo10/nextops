...
**Storage constraint: the owner reports only 3 TB.** Read the bilingual [storage plan](STORAGE_PLAN.md) before provisioning. Do not interpret that statement as 3 TB of verified free datastore space. Keep the existing 200/500/80-GiB starting VM disks and 36-vCPU/168-GiB RAM budget unless measured capacity requires a reviewed change; do not resize or shrink existing disks automatically. Count VM file/swap overhead, existing workloads, thin-disk growth commitments, snapshots, offline artifacts and temporary restore space. The proposed normal-operation headroom target is 25% of verified usable capacity, not a vendor guarantee. The first profile's 780-GiB VMDK total plus a provisional 168-GiB swap allowance is 948 GiB before other overhead. Record actual datastore capacity/free bytes and swap placement; the read-only `esxcli storage filesystem list` is the next missing storage observation. Preserve required local model/rollback files and the offline Zabbix milestone. No storage test has been run.
...
**محدودیت دیسک: مالک فقط ۳ ترابایت اعلام کرده است.** پیش از ساخت ماشین‌ها، [برنامهٔ دوزبانهٔ ذخیره‌سازی](STORAGE_PLAN.md) خوانده شود. این عدد، فضای آزاد تأییدشدهٔ datastore نیست. دیسک‌های اولیهٔ ۲۰۰، ۵۰۰ و ۸۰ GiB و مجموع ۳۶ vCPU و ۱۶۸ GiB حافظه حفظ شوند، مگر اینکه ظرفیت واقعی به تغییر بازبینی‌شده نیاز داشته باشد؛ دیسک موجود خودکار کوچک یا تغییر داده نشود. سربار فایل و swap ماشین، بار موجود، تعهد رشد دیسک thin، snapshot، فایل‌های آفلاین و فضای موقت بازیابی حساب شوند. حاشیهٔ آزاد پیشنهادی در کارکرد عادی ۲۵ درصد ظرفیت قابل‌استفادهٔ واقعی است، نه تضمین سازنده. جمع اولیهٔ ۷۸۰ GiB دیسک با سهم موقت ۱۶۸ GiB برای swap، پیش از سایر سربارها ۹۴۸ GiB می‌شود. ظرفیت و فضای آزاد واقعی و محل swap ثبت شوند؛ فرمان فقط‌خواندنی `esxcli storage filesystem list` مشاهدهٔ بعدیِ موردنیاز برای دیسک است. فایل‌های لازم مدل و بازگشت و هدف پاسخ آفلاین Zabbix حفظ شوند. هیچ آزمون ذخیره‌سازی اجرا نشده است.
