...
| Certificate lifecycle | **Local detection implemented; deployment/alert/rotation pending** | Use the dependency-free local checker and hardened daily timer with a 90-day warning window. Production still requires controlled installation, an owned local alert, trusted time and observed offline rotation/rollback. |
