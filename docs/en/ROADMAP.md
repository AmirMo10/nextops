...
**Status: the sequence is accepted and the complete controlled Phase 1 user-testing flow is deployed across four guests.** Local login, general CPU-only answers, read-only Zabbix evidence, durable investigation/audit, scoped failure recovery, four-guest WAN isolation, and serial clean reboots have evidence for the server/API path. This is not production acceptance; the independent fresh-browser offline case, sustained load, backup, isolated restore, and disaster recovery remain open. The [release manifest](../status/current-release.yaml) and [project state](../PROJECT_STATE.md) are authoritative for current gate status.
...
Phase 0 and the controlled Stage 1A–1E server/API path have the evidence recorded in [PROJECT_STATE](../PROJECT_STATE.md). The next bounded sequence is verified runtime/model/application rollback, independent backup and isolated restore, followed by the remaining browser and sustained-capacity cases. Later phases remain unaccepted unless an evidence-backed entry says otherwise. VM creation order does not require completing every feature of one guest before the next; contracts, scaffolding, a model hello-world, raw JSON, cached answers, or simulators alone never complete an acceptance gate.
